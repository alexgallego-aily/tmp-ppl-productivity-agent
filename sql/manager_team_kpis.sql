...
-- Source: data_normalized.ppl_employees + ppl_positions + ppl_organization_levels
-- Note: organization_level_code comes from ppl_positions → ppl_organization_levels.
--       Months where ppl_positions hasn't been synced yet will have NULL org_level
--       and are filtered out (the Python layer handles this gracefully).
-- Output: one row per (month, organization_level_code, geo_code)
...
WITH manager_employees AS (
    SELECT
        ...
        pe.hire_date,
        pe.continuous_hire_date,
        pe.position_start_date,
        pe.end_employment_date,
        pe.job_unit_code,
        pe.management_level_code,
        pe.extra,
        ROUND(
            ((pe.snapshot_date - COALESCE(pe.continuous_hire_date, pe.hire_date)) / 365.25)::NUMERIC, 1
        ) AS tenure_years,
        ROUND(
...
monthly_exits AS (
    SELECT
        DATE_TRUNC('month', pe.end_employment_date)::DATE AS exit_month,
        pol.organization_level_code,
        pe.geo_code,
        COUNT(DISTINCT pe.employee_code) AS exits
    FROM data_normalized.ppl_employees pe
    LEFT JOIN data_normalized.ppl_positions pp
        USING (snapshot_date, position_code)
    LEFT JOIN data_normalized.ppl_organization_levels pol
        USING (snapshot_date, organization_level_code)
    WHERE pe.manager_code = '{manager_code}'
      AND pe.end_employment_date IS NOT NULL
      AND pe.end_employment_date < CURRENT_DATE
      AND pe.is_artificial_record = FALSE
      AND pe.employee_time_type = 'Full time'
      AND pe.employee_type = 'Regular'
    GROUP BY 1, 2, 3
...
SELECT
    ...
    -- Context: dominant function and management level in this team
    ...
    ROUND(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY e.employee_salary_uom_value)::NUMERIC, 0) AS median_salary,
    ...
    -- Composite health score (avg of 3 components)
    ROUND(
        (
    ...
    -- ── 9. Risk ─────────────────────────────────────────────────────
    ROUND(SUM(CASE WHEN SPLIT_PART(SPLIT_PART(e.extra::TEXT, '"Retention": "', 2), '"', 1) = 'High'
                   THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 1) AS pct_high_retention_risk,

    ROUND(SUM(CASE
        WHEN SPLIT_PART(SPLIT_PART(e.extra::TEXT, '"Retention": "', 2), '"', 1) = 'High'
         AND SPLIT_PART(SPLIT_PART(e.extra::TEXT, '"Loss_Impact": "', 2), '"', 1) IN ('Critical', 'High')
